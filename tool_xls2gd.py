        ...
        for row_idx in range(row_idx, sheet.nrows):
            ...
            for col_idx in range(sheet.ncols):
                ...
                if type_dict[title] == INT and vtype == xlrd.XL_CELL_NUMBER:
                    ...
                    v = str(value)
                elif type_dict[title] == PYTHON and vtype in (xlrd.XL_CELL_TEXT, xlrd.XL_CELL_NUMBER):
                    v = str(value)
...
def write_to_gd_key(data, keys, type_dict, outfp, depth):
    ...
    for (key, value) in data.items():
        ...
        if depth == len(keys):
            ...
            write_to_gd_key(value, keys, type_dict,
                            outfp, depth + 1)
        cnt += 1
...
def write_to_gd_row(row, type_dict, outfp, depth):
    ...
    for (key, value) in row.items():
        if type_dict[key] == INT:
            ...
            outfp.write(template.format(
                indent, key, get_string_arr(value)))
